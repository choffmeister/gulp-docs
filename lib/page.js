...
module.exports.render = function (site) {
  ...
  return through.obj(
    function (file, _, cb) {
      try {
        eval(dust.compile(file.contents.toString(site.encoding), file.path));
        var context = {
          site: site,
          page: file.frontMatter
        };
        dust.render(file.path, context, function(err, out) {
          if (!err) {
            file.contents = new Buffer(out, site.encoding);
            this.push(file);
            cb();
          } else {
            cb(err);
